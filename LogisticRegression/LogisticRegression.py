...
def LogisticRegression():
    ...
    X = mapFeature(X[:, 0], X[:, 1])  # 映射为多项式 X:6*118
    initial_theta = np.random.normal(0, 4, X.shape[1]).reshape(-1, 1)  # 初始化theta
    # initial_theta = np.zeros((X.shape[1],1))
    print('初始theta:\n', initial_theta)

    initial_lambda = 0  # 初始化正则化系数，一般取0.01,0.1,1.....
    ...
    theta = computeGradientDescent(X, y, initial_theta, alpha=0.1, inital_lambda=initial_lambda, epc=1e-6)
    theta = theta.ravel()
    p = predict(X, theta)  # 预测
    p2 = predict(X, result)
    print('梯度下降法结果:\n', theta)
    print('用最优化函数结果：\n', result)
    print(u'梯度下降法在训练集上的准确度为%f%%' % np.mean(np.float64(p == y) * 100))  # 与真实值比较，p==y返回True，转化为float
    print(u'最优化函数法在训练集上的准确度为%f%%' % np.mean(np.float64(p2 == y) * 100))
    X = data[:, 0:-1]
    y = data[:, -1]
    plotDecisionBoundary(theta, X, y, '梯度下降法-')  # 画决策边界
    plotDecisionBoundary(result, X, y, '最优化函数法-')
...
def computeGradientDescent(X, y, theta, alpha, inital_lambda, epc):
    ...
    while (1):
        ...
        # print('X:',X.shape,'theta:',theta.shape,'h:',h.shape,'y:',y.shape)   #X: (118, 6) theta: (6, 1) h: (118, 1)e
...
# 画决策边界
def plotDecisionBoundary(theta, X, y, title=''):
    pos = np.where(y == 1)  # 找到y==1的坐标位置
    neg = np.where(y == 0)  # 找到y==0的坐标位置
    # 作图
    plt.figure(figsize=(15, 12))
    plt.plot(X[pos, 0], X[pos, 1], 'ro')  # red o
    plt.plot(X[neg, 0], X[neg, 1], 'bo')  # blue o
    plt.title(title + "决策边界", fontproperties=font)
